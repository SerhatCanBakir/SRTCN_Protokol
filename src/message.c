...
int srtcn_encode(const srtcn_message_t *msg, uint8_t *out_buf, size_t *out_len)
{
    ...
    if (msg->length > SRTCN_MAX_PAYLOAD_SIZE)
        return -2;
    memcpy(out_buf + 5, msg->Payload, msg->length);
...
{
if(!in_buff||in_len<5||!out_msg)return -1;

out_msg->type = in_buff[0];
out_msg->message_id = strcn_ntohs(*(uint16_t*)(in_buff+1));
out_msg->length = strcn_ntohs(*(uint16_t*)(in_buff+3));

if(out_msg->length > SRTCN_MAX_PAYLOAD_SIZE|| in_len < 5 + out_msg->length) return -2;
memcpy(out_msg->Payload,in_buff+5,out_msg->length);
return 0;
}
